...
export class CambioIdioma {
  ...
  async detectarYConfigurar() {
    ...
    setInterval(async () => {
      ...
      if (this.lecturaAutomatica && this.appActiva && this.tienePermisos && this.varPrueba) {
        try {
          let lat: number;
          let lon: number;
  ...
  const paisesES = ['AR', 'ES', 'MX', 'CL', 'CO', 'PE', 'UY'];
  const paisesPT = ['PT', 'BR', 'AO', 'MZ', 'MO'];
  const paisesDE = ['DE', 'AT', 'CH', 'LI'];
  const paisesFR = ['FR', 'BE', 'MC'];
  const paisesRU = ['RU', 'BY', 'KZ', 'KG'];
  const paisesEN = ['US', 'GB', 'AU', 'NZ', 'IE', 'HK'];
