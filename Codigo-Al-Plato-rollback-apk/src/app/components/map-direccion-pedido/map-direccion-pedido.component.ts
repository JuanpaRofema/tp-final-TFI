import { AfterViewInit, Component, CUSTOM_ELEMENTS_SCHEMA, inject, ViewChild, ElementRef, signal } from '@angular/core';
...
export class MapDireccionPedidoComponent implements AfterViewInit {
  ...
  initMap() {
    ...
    this.map.on('click', async (e: L.LeafletMouseEvent) => {
      ...
      this.direccion = this.diccionario[this.idioma()]['BuscandoCalle'];
  ...
  async buscarDireccion() {
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        this.mostrarError(this.diccionario[this.idioma()]['DireccionNoEncontrada']);
      }

    } catch (error) {
      console.error('Error:', error);
      this.mostrarError(this.diccionario[this.idioma()]['ErrorConexion']);
    } finally {
      this.buscando = false;
    }
  }
  ...
  confirmarUbicacion() {
    ...
    Swal.fire({
      title: this.diccionario[this.idioma()]['UbicaciónGuardada'],
      icon: 'success',
      timer: 1500,
  ...
  async obtenerDireccion(lat: number, lng: number) {
    try {
      ...
      if (data && data.display_name) {
        ...
        this.direccion = this.diccionario[this.idioma()]['UbicacionSinNombre'];
      }
