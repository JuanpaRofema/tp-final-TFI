    ...
    `,
      ...
      didOpen: () => {
        ...
        document.getElementById('btn-geo')?.addEventListener('click', () => {
          Swal.close();
          this.cambioIdioma.cambiarIdiomaAutomatico()
        });
      }
    });
  }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (this.cordenadasElegidas) {
          let idiomaActual = this.idioma()
          console.log(idiomaActual, "es esoooo!")
          let resultado: any = this.cambioIdioma.cambiarIdiomaManual(this.cordenadasElegidas);
          if (resultado[1]) {
            const alertasIdioma: any = {
              es: { titulo: 'Idioma modificado correctamente', descripcion: 'El idioma se ha cambiado a Español', boton: 'Aceptar' },
              en: { titulo: 'Language modified successfully', descripcion: 'The language has been changed to English', boton: 'Accept' },
              pt: { titulo: 'Idioma modificado com sucesso', descripcion: 'O idioma foi alterado para Português', boton: 'Aceitar' },
              de: { titulo: 'Sprache erfolgreich geändert', descripcion: 'Die Sprache wurde auf Deutsch geändert', boton: 'Bestätigen' },
              fr: { titulo: 'Langue modifiée avec succès', descripcion: 'La langue a été changée en Français', boton: 'Accepter' },
              ru: { titulo: 'Язык успешно изменен', descripcion: 'Язык был изменен на Русский', boton: 'Принять' }
            };
            Swal.fire({ title: alertasIdioma[resultado[0]].titulo, text: alertasIdioma[resultado[0]].descripcion, icon: 'success', confirmButtonText: alertasIdioma[resultado[0]].boton});
          }
          else {
            const erroresIdioma: any = {
              es: { titulo: 'Idioma incorrecto o no soportado', descripcion: 'El país del idioma no está soportado o es una región incorrecta', boton: 'Cerrar' },
              en: { titulo: 'Incorrect or unsupported language', descripcion: "The language's country is not supported or it is an incorrect region", boton: 'Close' },
              pt: { titulo: 'Idioma incorreto ou não suportado', descripcion: 'O país do idioma não é suportado ou é uma região incorreta', boton: 'Fechar' },
              de: { titulo: 'Falsche oder nicht unterstützte Sprache', descripcion: 'Das Land der Sprache wird nicht unterstützt oder es ist eine falsche Region', boton: 'Schließen' },
              fr: { titulo: 'Langue incorrecte ou non supportée', descripcion: "Le pays de la langue n'est pas supporté ou c'est une région incorrecte", boton: 'Fermer' },
              ru: { titulo: 'Неверный или неподдерживаемый язык', descripcion: 'Страна языка не поддерживается или указан неверный регион', boton: 'Закрыть' }
            };
            Swal.fire({ title: erroresIdioma[idiomaActual].titulo, text: erroresIdioma[idiomaActual].descripcion, icon: 'error', confirmButtonText: erroresIdioma[idiomaActual].boton});
          }
        }
        else {
